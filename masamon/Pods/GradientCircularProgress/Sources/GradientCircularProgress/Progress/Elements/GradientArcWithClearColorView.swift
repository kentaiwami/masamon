...
class GradientArcWithClearColorView: UIView {
    ...
    internal func draw(_ rect: CGRect, prop: Property) -> UIImageView {
        ...
        // StartArc
        startArcColorProp.endArcColor = ColorUtil.toNotOpacityColor(startArcColorProp.startArcColor)
        
        // EndArc
        endArcColorProp.startArcColor = ColorUtil.toNotOpacityColor(endArcColorProp.endArcColor)
        ...
        /* Composite */
        let image: UIImage = composite(startArcImage, image2: endArcImage, prop: prop)
    ...
    internal func composite(_ image1: UIImage, image2: UIImage, prop: Property) -> UIImage {
        ...
        image1.draw(
            ...
            blendMode: .overlay,
            alpha: ColorUtil.toRGBA(prop.startArcColor).a)
        image2.draw(
            in: CGRect(x: 0, y: 0, width: image2.size.width, height: image2.size.height),
            blendMode: .overlay,
            alpha: ColorUtil.toRGBA(prop.endArcColor).a)
        let image = UIGraphicsGetImageFromCurrentImageContext()
        UIGraphicsEndImageContext()
