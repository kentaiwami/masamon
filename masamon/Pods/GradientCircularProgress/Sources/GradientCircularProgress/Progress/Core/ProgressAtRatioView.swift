...
class ProgressAtRatioView: UIView {
    ...
    internal func initialize(_ frame: CGRect) {
        ...
        // Gradient Circular
        if ColorUtil.toRGBA(prop.startArcColor).a < 1.0 || ColorUtil.toRGBA(prop.endArcColor).a < 1.0 {
            // Clear Color
            let gradient: UIView = GradientArcWithClearColorView().draw(rect, prop: prop)
            addSubview(gradient)
            
            masking(rect, prop: prop, gradient: gradient)
    ...
    fileprivate func masking(_ rect: CGRect, prop: Property, gradient: UIView) {
        // Mask
